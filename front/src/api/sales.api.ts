...
export const salesApi = {
  ...
  //sales/statistics
  getStatistics: async (): Promise<SaleStatistics | null> => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/sales/statistics`, {
        method: "GET",
      });
      if (!res.ok)
        throw new Error(`Network response was not ok: ${res.status}`);
      ...
      console.error("salesApi.getStatistics error:", error);
      return null;
    }
  },
